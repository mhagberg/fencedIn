...
if (Meteor.isServer) {
    ...
    Meteor.publish('jobs', function jobsPublication() {
        let jobs = Jobs.find({$and:[{hidden : null}, {$or:[{'finishDate': null}, {'finishDate': ""}]}]}, {sort : {createDate : -1}, limit:3000}, {createDate : 1, name : 1, number: 1});
        return jobs;
    });

    Meteor.publish('hiddenJobs', function hiddenJobsPublication() {
        let jobs = Jobs.find({'hidden' : true},  {sort:{createDate:-1}, limit:3000}, { name: 1, number: 1, createDate: 1, finishDate: 1});
        return jobs;
    });

    Meteor.publish('job_Reports', function jobReportsPublication() {
        let jobs = Jobs.find({'finishDate' : {$exists:true, $ne:""}}, {sort:{'finishDate':-1},  limit:3000}, { name: 1, number: 1, createDate: 1, finishDate: 1});
        return jobs;
    });
